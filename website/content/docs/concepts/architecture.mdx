...
MCP host
  ...
  └─ Streamable HTTP ───────┤
           ...
           ┌─────────────────┴─────────────────┐
           ▼                                   ▼
  native intelligence tools                 file lifecycle tools
           │                                   │
           └──────────── root policy ──────────┘
                                               │
                                  patch state machine
...
The installed CLI resolves native intelligence dependencies and dprint from the package's stable dependency tree. The source and templates remain in the package because the runtime, installer, and generated host surfaces reference them.
