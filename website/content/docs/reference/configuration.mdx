...
Secure defaults preserve the workspace boundary while allowing file operations in the OS temporary directory: arbitrary paths and external configured roots are rejected, symbolic links are rejected, mutation hashes are required, and the hook remains enabled. Set `allow_temp_directory = false` to remove the default temporary-directory allowance. Set `allow_any_path = true` only for explicit unrestricted file-tool access; it bypasses workspace and temporary-directory path protection but does not disable hash, regular-file, locking, or symlink policy. `follow_symlinks = true` permits a final symlink only when its resolved target remains inside an allowed root for that surface: workspace roots for AST intelligence, and workspace or enabled temporary-directory roots for file tools. Unrestricted file mode permits any resolved target. `require_hash = false` makes `expectedSha256` optional for `file_patch`, existing-file `file_write`, `file_rename`, and `file_delete`; a supplied hash is always verified.
...
| `AST_MCP_ALLOW_ANY_PATH`            | `safety.allow_any_path`          |
| `AST_MCP_ALLOW_EXTERNAL_ROOTS`      | `safety.allow_external_roots`    |
| `AST_MCP_ALLOW_TEMP_DIRECTORY`      | `safety.allow_temp_directory`    |
