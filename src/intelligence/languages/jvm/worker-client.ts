...
export function validateJvmWorkerResponse(
  value: unknown,
  ...
  if (expected && response.id !== expected.id) {
    throw new JvmWorkerError("protocol", "JVM worker response id mismatch");
  }
...
export function rejectPendingJvmRequests(error: unknown): void {
...
export function handleJvmWorkerError(active: Worker, error: Error): void {
  if (worker === active) rejectPendingJvmRequests(error);
}
async function terminateWorker(
  active: Worker | undefined,
  failure: JvmWorkerError,
  canceledId?: number,
): Promise<void> {
  ...
  if (!termination) {
    ...
    termination = (async () => {
      ...
      rejectPendingJvmRequests(failure);
    })().finally(() => {
...
function parserWorker(): Worker {
  ...
  active.on("message", (value: unknown) => {
    try {
      const response = validateJvmWorkerResponse(value);
      const pendingRequest = pending.get(response.id);
      if (!pendingRequest) return;
      const validated = validateJvmWorkerResponse(value, {
        id: response.id,
      ...
      void terminateWorker(
        ...
        new JvmWorkerError(
          "protocol",
          "JVM parser worker returned an invalid response",
        ),
      );
    }
  });
  active.on("error", handleJvmWorkerError.bind(undefined, active));
  active.on("exit", (code) => {
    if (worker !== active) return;
    rejectPendingJvmRequests(
      new JvmWorkerError(
        "worker-exit",
        `Jvm parser worker exited with code ${code}`,
      ),
    );
  });
  return active;
...
export function analyzeJvmLanguage(
  ...
  if (termination) {
    return termination.then(() => analyzeJvmLanguage(request));
  }
  const bytes = Buffer.byteLength(request.source);
  if (
    pending.size >= JVM_WORKER_LIMITS.maxPendingRequests ||
    outstandingBytes + bytes > JVM_WORKER_LIMITS.maxOutstandingBytes
  ) {
    return Promise.reject(
      new JvmWorkerError("queue-full", "Jvm parser capacity exceeded"),
    );
  }
  if (request.signal?.aborted) {
    return Promise.reject(
      new JvmWorkerError("aborted", "Jvm parser request aborted"),
    );
  }
  const id = nextId++;
  const timeoutMs = request.timeoutMs ?? JVM_WORKER_LIMITS.timeoutMs;
  return new Promise<SyntaxFacts>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      void terminateWorker(
        worker,
        new JvmWorkerError("timeout", "Jvm parser request timed out"),
        id,
      );
    }, timeoutMs);
    const cleanup = () => {
...
export async function interruptJvmLanguageWorker(): Promise<void> {
  await terminateWorker(
    worker,
    new JvmWorkerError("worker-exit", "Jvm parser worker exited unexpectedly"),
  );
}
export async function closeJvmLanguageWorker(): Promise<void> {
  if (termination) await termination;
  await terminateWorker(
    worker,
    new JvmWorkerError("closed", "Jvm parser worker closed"),
  );
}
export async function restartJvmLanguageWorker(): Promise<void> {
