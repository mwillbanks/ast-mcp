...
function send(result: LegacyWorkerResult): void {
...
async function wasm(
  ...
  for (const collection of [
    ...
    for (const item of collection) {
      item.start = byteToUtf16(source, item.start);
      item.end = byteToUtf16(source, item.end);
    }
  }
  tree.delete();
  parser.delete();
...
function buildFacts(
  ...
  const extractorFingerprint = legacyGrammarFingerprint(languageId);
  const facts: SyntaxFacts = {
    calls,
    diagnostics,
    exports: [],
    extractorFingerprint,
    grammarFingerprint: extractorFingerprint,
    implementations: [],
    imports,
    inheritance,
    languageId,
...
parentPort?.on("message", async (value: unknown) => {
  ...
  queued++;
  try {
    const facts = await parse(value);
    if (!canceled.delete(value.id))
      send({ facts, id: value.id, ok: true, type: "result" });
  } catch (error) {
    send({
