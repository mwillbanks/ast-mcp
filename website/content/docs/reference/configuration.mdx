...
kind = "global" # global | local | parent | explicit
...
## Intelligence storage and models

All index state uses LanceDB. Global placement shares content-addressed artifacts across repositories and revisions. Local placement stores data under one repository. Parent placement requires `levels`; explicit placement requires an absolute `path`. Cross-repository federation remains disabled unless `intelligence.federation.enabled = true`.

Semantic retrieval is optional. The default pinned Granite embedding profile is small and CPU-oriented. Override the model, revision, dimensions, dtype, pooling, device, workers, queue, batch size, and local artifact path together. Generation is disabled by default and never blocks structural tools. MCP providers use the active host sampling capability; HTTP providers require an explicit endpoint, model, and optional environment-variable name for the API key.
...
- `config_core` updates `workspace`, `safety`, `files`, `formatting` scalars (`enabled`, `fallback`, `dprint_config`), `intelligence`, `http`, `dependencies`, and `mcp.configuration`. Batch every key that belongs to those sections in one call. It does not change `[[paths]]` or `[[formatting.formatters]]`.
