...
- `file_write({ files: { "/repo/a.txt": { content, chattr? }, "/repo/b.txt": { content, expectedSha256?, chattr? } } })`: create or replace multiple files in one declared batch; existing replacements remain limited to non-AST-rewritable files. Missing parents are created only after the write attempt returns `ENOENT`, then the path is revalidated.
- `file_patch({ files: { "/repo/src/a.ts": { expectedSha256?, patchStrategy: "ast", astRules: [...], chattr? } } })`: patch one or more files in one declared batch. Use ordered `astRules` or `aiderBlocks`; every path has one atomic commit.
...
- Mutation tools use a declared `files` input object and return a `files` result map. The OS temporary directory is allowed by default; configure `safety.allow_temp_directory = false` to disable it or `safety.allow_any_path = true` for explicit unrestricted file access. MCP transport also accepts JSON-RPC request/notification arrays and returns one response per request ID.
