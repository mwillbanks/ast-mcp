...
`projectRoot` remains the config-bearing checkout. Intelligence calls that target an absolute worktree path use that worktree as the native intelligence root. Relative paths resolve against `PWD` when it is a linked worktree.
