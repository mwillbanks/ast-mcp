...
Targets are `codex`, `claude`, `copilot`, or `all`. The installer records the invoked executable and omits MCP environment fields. Local installation creates project configuration. Global installation creates project-neutral user configuration.

## ast-bro platform support

`@ast-bro/cli@4.0.0` currently publishes a precompiled binary only for macOS Apple Silicon. Before changing host configuration, the ast-mcp installer verifies that the pinned ast-bro binary exists and reports the expected version.
